  ...
  test "correctly handles a value with brackets [ ]" do
    {:ok, parse_result} =
      ConfigParser.parse_string("""
        ...
        paired_spaces = [ value ]
        odd_spaces = [ yes]
        no_spaces = [without spaces]
      """)

    assert ConfigParser.get(parse_result, "brackets_value", "paired_spaces") == "[ value ]"
    assert ConfigParser.get(parse_result, "brackets_value", "odd_spaces") == "[ yes]"
    assert ConfigParser.get(parse_result, "brackets_value", "no_spaces") == "[without spaces]"
  end
